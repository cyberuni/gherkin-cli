...
Feature: parse — the projection engine
  ...
  Scenario: default projection lists scenarios with name, keyword, and tags
    ...
    When parse runs on it with default options
  ...
  Scenario: full adds step and example detail
    ...
    When parse runs on it with full true
  ...
  Scenario: tag keeps only scenarios carrying the tag
    ...
    When parse runs on it with tag @frozen
  ...
  # ── ast ─────────────────────────────────────────────────────────
  Scenario: parseAst returns the raw GherkinDocument
    Given a well-formed .feature file
    When parseAst runs on it
  ...
  Scenario: the file text is read through an injected reader
    Given a ReadsFile that returns feature text without touching the filesystem
    When parse runs with that reader injected
