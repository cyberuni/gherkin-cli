...
`parse(paths, {full?, tag?})` reads one or more `.feature` files and returns a compact `ParseResult` a consumer can reason over cheaply: per file the feature tags, scenario count, and section-comment count; per scenario the name, keyword, and tags. Detail (step text, step/example counts) is opt-in via `{full: true}`. `parseAst(paths)` returns the raw cucumber `GherkinDocument` per file. Both are pure — a missing or malformed file becomes an `error` entry rather than throwing, so the batch always returns.
...
- A tool wants the raw AST → `parseAst`.
...
- `parseAst` returns the raw `GherkinDocument` per file.
...
- The filesystem seam is a separate 3rd `deps` argument — a `ReadsFile` role interface (default `nodeReadsFile`), not an option. Pass a fake `deps` (`{ readFile }`) to drive the engine from in-memory text with no disk access. `parseAst` takes the same `deps` argument.
