...
- The filesystem seam is a separate 3rd `deps` argument — a `ReadsFile` role interface (default `nodeReadsFile`), not an option. Pass a fake `deps` (`{ readFile }`) to validate in-memory text with no disk access.
